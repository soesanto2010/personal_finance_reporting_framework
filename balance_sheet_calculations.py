...
def get_account_level_balance_sheet(self):
    ...
    df = baseline_amount.merge(delta_amount,left_on=["acc_ID"],right_on=["Impacted_Acc_ID"],how="left")
    ...
    self.Acct_Level_Summary = df.merge(self.securities[["acc_ID","MV"]],on=["acc_ID"],how="left")
    self.Acct_Level_Summary.rename(columns={"acc_baseline_value":"Baseline_Value",
    ...
    self.Acct_Level_Summary.loc[self.Acct_Level_Summary["acc_name"].str.contains("Unrealized Investment Gain"),"MV"] = Unrealized_gain
    ...
    self.Acct_Level_Summary.loc[self.Acct_Level_Summary["acc_name"].str.contains("Unrealized Investment Loss"),"MV"] = Unrealized_loss * -1
    ...
    self.Acct_Level_Summary = self.Acct_Level_Summary[(self.Acct_Level_Summary["Baseline_Value"] != 0) | (self.Acct_Level_Summary["MV"] != 0)]
