...
def verify_accounting_equation(self):
    ...
    LHS = (
        self.Class_Level_Summary[
            ...
            & (self.Class_Level_Summary["acc_A_L_E_sign"] == "[+ve]")
        ]["MV"].sum()
        - self.Class_Level_Summary[
            (self.Class_Level_Summary["acc_A_L_E_classification"] == "Asset")
            & (self.Class_Level_Summary["acc_A_L_E_sign"] == "[-ve]")
...
def ensure_closing_date_is_later_than_initiation_dates(self):
    ...
    Non_chronological_dates = len(
        self.Transactions_temp_2[
            self.Transactions_temp_2["tr_close_date"]
            < self.Transactions_temp_2["tr_init_date"]
        ]
    )
...
def ensure_no_non_standard_account_names(self):
    ...
    Non_standard_acc_names = len(
        self.Transactions_temp_2[
            (self.Transactions_temp_2["Impacted_Acc_ID_1"].isnull())
            | (self.Transactions_temp_2["Impacted_Acc_2"].isnull())
        ]
    )
    Non_standard_rev_names = len(
        self.transactions_with_income_IDs[
            self.transactions_with_income_IDs["inc_grp_ID"].isnull()
        ]
    )
    Non_standard_exp_names = len(
        self.transactions_with_expense_IDs[
            self.transactions_with_expense_IDs["exp_grp_ID"].isnull()
        ]
    )
